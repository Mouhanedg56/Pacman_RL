...
from layers import NoisyNetDense
...
class QnetworkNoisy():
	def __init__(self, h_size, n_actions):
		...
		self.denseA = NoisyNetDense(h_size//2, activation='relu')(self.streamA)
		self.denseV = NoisyNetDense(h_size//2, activation='relu')(self.streamV)
		self.Advantage = NoisyNetDense(n_actions, activation='linear')(self.denseA)

		xavier_init = tf.contrib.layers.xavier_initializer()
		self.VW = tf.Variable(xavier_init([h_size//2, 1]))
		self.Value = tf.matmul(self.denseV, self.VW)
